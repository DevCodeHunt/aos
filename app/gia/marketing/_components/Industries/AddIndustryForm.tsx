...
import { Project, Service, TImage } from "@/types";
import { useToast } from "@/components/ui/use-toast";
import { addIndustry, addProject } from "@/actions";
      ...
      try {
        const responseData = {
          ...
          description: values.description,
           projects: values.projects?.filter((project): project is { id: string; value: string; label: string } => !!project && 'id' in project && 'value' in project && 'label' in project).map((project) => project.id) || [],
          services: values.services.map((service) => service.id),
          banner: banner?.secure_url || "",
          thumbnail: thumbnail?.secure_url || "",
        };

        const data = await addIndustry(responseData);
        ...
        <Controller
          ...
          render={({ field: { value, onChange }, fieldState: { error } }) => (
            <div>
              ...
              <MultiSelect
                options={allProjects}
                value={value?.filter(item => item !== undefined) as { label: string; value: string; id?: string }[]}
                onChange={onChange}
                error={error?.message}
              />
            </div>
